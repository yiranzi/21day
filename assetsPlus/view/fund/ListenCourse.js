...
const ListenCourse = React.createClass({
    ...
    componentWillMount() {
        ...
        //音频完成监听
        OnFire.on('AUDIO_END',()=>{
            if (this.state.currentPlaying<0 || sessionStorage.getItem('courseId') !== '1') {
                return null;
            }
            //终止多余的其你去
            if(!this.state.timer){
    ...
    preLoadPic() {
        return(<div className="pre-load">
            {/*<img src={'./assetsPlus/image/course/bglight_b.png'}/>*/}
        </div>)
    },
                ...
                if(i === 0 || lessons[i-1].subs[(lessons[i-1].subs.length) - 1].process) {
                    ...
                    if(lessons[i].process){
                        ...
                        if(lessonQuestions[lessonQuestions.length - 1].process && i !== lessons.length - 1) {
                            arr.push(<div style = {{backgroundImage: 'url("./assetsPlus/image/fund/DividingLine.png")'}} className="lesson-column-line-fund"></div>);
                            count++;
                        }
                    } else break OUT;
